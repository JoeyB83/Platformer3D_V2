...
public class EnemyController : MonoBehaviour
{
    ...
    public float waitBeforeDieing = .5f, squashSpeed;
    private float dieingCounter;
    ...
    void Update()
    {
        if(dieingCounter > 0)
        {
            dieingCounter -= Time.deltaTime;
            theRB.velocity = new Vector3(0f, theRB.velocity.y, 0f);

            transform.localScale = Vector3.Lerp(transform.localScale, new Vector3(1.25f, 0.5f, 1.25f), squashSpeed * Time.deltaTime);
    ...
    {
        if(collision.gameObject.tag == "Player" && dieingCounter == 0)
        {
    ...
    private void OnTriggerEnter(Collider other)
    {
        if(other.tag == "Player")
        {
            ...
            dieingCounter = waitBeforeDieing;
